...
window.jsBridge = { status: { loadstatus: false } }
const _bridgeProps = {
  status: false,
  value: ''
}
const _bridges = [
  'images', 'idCard', 'address', 'bank', 'job', 'customer', 'sign', 'viewPdf', 'shareInvoke', 'audio',
  {'camera': { index: 1 }},
  {'search': {callback: () => {}}}
]
_bridges.forEach((item) => {
  if (typeof item === 'string') {
    window.jsBridge.status[item] = _bridgeProps
  } else {
    let key = Object.keys(item)[0]
    let value = Object.values(item)[0]
    window.jsBridge.status[key] = Object.assign({..._bridgeProps}, value)
  }
})
...
// 打开搜索界面
window.openSearch = function (type, hint, message) {
  if (!type) {
    alert('搜索类型为必要字段！')
  }
...
window.app2js_onDataResult = function (type, data) {
  ...
  switch (type) {
    ...
    case 'takeUserImageMultiple':
      ...
      break
    case 'search':
      window.jsBridge.status.search.value = data
      window.jsBridge.status.search.status = true
      window.jsBridge.status.search.callback(data)
      break
  }
}
// 3.2 关闭webview
...
export const setHeadBar = function ({hide}) {
  if (window.HQAppJSInterface && window.HQAppJSInterface.hideSearchBox) {
    window.HQAppJSInterface.hideSearchBox(hide)
  }
}
